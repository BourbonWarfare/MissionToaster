...
};
{
	...
	// systemChat format ["%1 = %2", _factions, count _loadoutClasses];
	{
		...
		// diag_log text format ["Looking at %1>>%2", _faction, configName _loadoutPath];
		{
			...
			[_allWeapons, ["uniform","vest","headgear","weapons","launchers","handguns","items","linkedItems"]],
...
diag_log text format ["END - test.sqf - [%1]", diag_tickTime]; // diag_tickTime gives insight to time spent loading config
nil
